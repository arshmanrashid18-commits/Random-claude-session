/**
 * Seeded 3D simplex noise plus fractal helpers (fbm, ridged, domain warp).
 * Based on Stefan Gustavson's public-domain simplex noise, with the permutation
 * table derived from our deterministic Rng so every world seed differs.
 */
import { Rng } from './rng';
...
// 12 gradient directions (edges of a cube), flattened.
...
export class Noise3 {
  ...
  constructor(seed: number | string) {
    const rng = new Rng(typeof seed === 'string' ? seed : seed ^ 0x51f15e);
  ...
  fbm(x: number, y: number, z: number, octaves: number, lacunarity = 2.0, gain = 0.5): number {
    ...
    for (let o = 0; o < octaves; o++) {
      sum += amp * this.noise(x * freq + o * 17.13, y * freq - o * 9.71, z * freq + o * 3.33);
      norm += amp;
      amp *= gain;
      freq *= lacunarity;
    }
  ...
  /**
   * Ridged multifractal in [0,1]: sharp crests where noise crosses zero.
   * Successive octaves are weighted by the previous octave to concentrate
   * detail on ridges (Musgrave).
   */
  ridged(x: number, y: number, z: number, octaves: number, lacunarity = 2.1, gain = 0.5): number {
    ...
    let norm = 0;
    for (let o = 0; o < octaves; o++) {
      let n = 1 - Math.abs(this.noise(x * freq + o * 5.1, y * freq + o * 1.7, z * freq - o * 7.3));
      n *= n;
      n *= weight;
