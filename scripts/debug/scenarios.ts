...
import { WIN, LOSE, IDLE, play } from '../../tests/support/strategies';
const only = process.argv[2] ? process.argv[2].split(',') : null;
const mode = process.argv[3] ?? 'both';
for (const def of SCENARIOS) {
  if (only && !only.includes(def.id)) continue;
  for (const kind of mode === 'both' ? ['win', 'lose'] : mode === 'all' ? ['win', 'lose', 'idle'] : [mode]) {
    const t0 = performance.now();
    const w = new World({ seed: def.seed, preset: def.preset });
    startScenario(w, def.id);
    const r = play(w, kind === 'win' ? WIN[def.id] : kind === 'idle' ? IDLE : LOSE[def.id], def.years + 1);
